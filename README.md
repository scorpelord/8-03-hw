...
После уничтожения ресурсов файл terraform.tfstate должен быть пустым или содержать минимальную информацию о состоянии без активных ресурсов.
Почему не был удалён docker-образ nginx:latest :
Docker-образ nginx:latest не был удалён, потому что Terraform управляет только зависимыми ресурсами, которые были явно созданы в процессе. В коде не указано, что образ должен быть удалён при уничтожении контейнера.
В документации провайдера Docker для ресурса docker_image указывается, что Terraform управляет только состоянием для образов, связанных с контейнерами, которые он создаёт и уничтожает. Образы, которые не связаны напрямую с контейнерами в конфигурации, остаются на месте.
```
